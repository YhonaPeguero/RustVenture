...
export const ProgressBar = ({ current, total, color = '#f97316' }) => (
...
export const CodeBlock = ({ code, language = 'rust', highlightLine = -1 }) => (
  ...
  >
    <code className="text-[#f1f5f9]">
      {code.split('\n').map((line, i) => (
        <div key={i} className={`flex ${highlightLine === i ? 'bg-[#f97316]/20 -mx-4 sm:-mx-5 px-4 sm:px-5 border-l-2 border-[#f97316]' : ''}`}>
...
export const CrabMascot = ({ message }) => (
  ...
  >
    ...
    <div className="space-y-1">
        <p className="text-xs font-bold text-[#f97316] uppercase tracking-widest">Ferris el Cangrejo</p>
